...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between mb-3">
        <h2>Data Pesanan</h2>
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalTambah">+ Tambah Pesanan</button>
    </div>
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modalDetail{{ $item->id_pesanan }}">Detail</button>

                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#modalEdit{{ $item->id_pesanan }}">Edit</button>
                    
                    <form action="{{ route('pesanan.destroy', $item->id_pesanan) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data pesanan ini?')">Hapus</button>
                    </form>
                </td>
            </tr>

            <div class="modal fade" id="modalEdit{{ $item->id_pesanan }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Edit Pesanan: {{ $item->nama_pelanggan }}</h5>
